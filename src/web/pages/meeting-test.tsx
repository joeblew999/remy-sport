...
export function MeetingTestPage({ route, goto }: { route: Route; goto: (route: Route) => void }) {
  return <>
    <PageHeader title={m.meeting_title()} />
    <PageInner>
      {__BUILD__.environment !== "dev" ? <EmptyState>{m.meeting_dev_only()}</EmptyState> :
...
function MeetingLobby({ route, goto }: { route: Route; goto: (route: Route) => void }) {
  ...
  if (!user) return <Card><CardHeader><CardTitle>{m.meeting_title()}</CardTitle><CardDescription>{m.meeting_sign_in()}</CardDescription></CardHeader><CardFooter>
    <ButtonLink href={routeHref(signInRoute(route))}>{m.sign_in()}</ButtonLink>
  </CardFooter></Card>
  return <div className="flex flex-col gap-4">
    <Card data-testid="meeting-lobby">
      <CardHeader><CardTitle>{m.meeting_title()}</CardTitle><CardDescription>{m.meeting_experiment()}</CardDescription></CardHeader>
      {valid && <CardContent><Field>
        <FieldLabel htmlFor="meeting-invite">{m.meeting_invite()}</FieldLabel>
        <Input id="meeting-invite" readOnly value={invite} onFocus={event => event.target.select()} />
        <FieldDescription>{m.meeting_invite_hint()}</FieldDescription>
      </Field></CardContent>}
      <CardFooter className="flex flex-wrap gap-3">
        {!valid ? <Button onClick={create} data-testid="meeting-create">{m.meeting_create()}</Button> : <>
          <Button variant="outline" onClick={() => void copy()}>{copied ? m.meeting_copied() : m.meeting_copy()}</Button>
          <Button onClick={() => setJoined(!joined)} data-testid="meeting-join">{joined ? m.meeting_leave() : m.meeting_join()}</Button>
        </>}
      </CardFooter>
    </Card>
    {copyError && <Alert><AlertDescription>{m.meeting_copy_failed()}</AlertDescription></Alert>}
    {joined && relay.isPending && <Loading />}
    {joined && relay.isError && <Alert variant="destructive"><AlertDescription>{formErrors(relay.error).form ?? m.meeting_connect_failed()}</AlertDescription><Button variant="outline" onClick={() => void relay.refetch()}>{m.push_retry()}</Button></Alert>}
    ...
    {joined && config && <div className="grid gap-4 lg:grid-cols-2" data-testid="meeting-media">
      ...
      <MoqWatcher name={config.watch.name} config={config.watch} title={m.meeting_peer()} description={m.meeting_peer_hint()} />
    </div>}
  </div>
}
