...
export type IconName =
  | "discover" | "teams" | "live" | "standings" | "profile"
...
export function Icon({ name }: { name: IconName | string }) {
  const path = (PATHS as Record<string, JSX.Element>)[name];
  return (
    <svg viewBox="0 0 16 16" width="16" height="16" style={{ display: "inline-block", flexShrink: 0, verticalAlign: "middle" }}>
      {path ?? null}
    </svg>
  );
}
