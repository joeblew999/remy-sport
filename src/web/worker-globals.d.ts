...
/**
 * Stubs for the Worker globals `src/types.ts` names.
 *
 ...
 * names `D1Database`, `R2Bucket`, `Fetcher`, `SendEmail` and
 * `AnalyticsEngineDataset` — ambient globals declared by
 * `worker-configuration.d.ts`, which is in the Worker's tsconfig and
 * deliberately NOT in this one. Without these lines the SPA cannot
