 ...
 * present. See docs/2026-09-09-16-pretranslated-reference-data.md for what the
...
import { ALL_LOCALES, type Locale } from "../../src/domain/model/vocabularies.ts"

const CACHE = join(tmpdir(), "remy-refdata")
...
const [command = "score", ...names] = process.argv.slice(2)
if (command !== "score") {
  console.log("usage: bun run ops refdata score [candidate...]\n\ncandidates:")
  for (const [name, c] of Object.entries(CANDIDATES)) console.log(`  ${name.padEnd(10)} ${c.licence}  (downloads ${c.weight})`)
  process.exit(1)
}

const wanted = names.length ? names : Object.keys(CANDIDATES)
console.log(`Scored against the ${ALL_LOCALES.length} locales the model declares: ${ALL_LOCALES.join(" ")}`)
console.log("A percentage is rows carrying a name in that locale. An em dash is a locale the dataset does not offer at all.")
let failed = false
