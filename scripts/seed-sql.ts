 ...
 *
 * Without it the team write path is unreachable: a coach holds the platform-wide
 * `team:update` permission but stands in no relation to any specific school,
 * which is exactly the distinction `requireOrgMember` enforces.
 *
 * Coach joins Assumption College as an `admin` so the data exercises both write
 * tiers — member-level create/update and admin-level delete. Organizer joins
...
lines.push("", "-- Memberships — who may write for which school.")
