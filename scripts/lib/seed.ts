 ...
 *
 * The vocabularies already work this way: `bun run ops domain` turns the
...
const lines: string[] = [
  "-- GENERATED by `bun run model` (its seed step) — do not edit.",
  "--",
...
if (process.argv.includes("--check")) {
  ...
  if (spawnSync("git", ["diff", "--quiet", "--", "src/db/seed.sql"]).status !== 0) {
    console.error("check-seed: seed.sql is stale — run 'bun run model' and commit the result:")
