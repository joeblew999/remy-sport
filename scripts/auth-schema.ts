 ...
 *
 *   bun scripts/auth-schema.ts            fail if src/db/auth-schema.ts is stale
 *   bun scripts/auth-schema.ts --write    regenerate it
 *
...
try {
  ...
  if (readFileSync(fresh, "utf-8") !== before) {
    console.error(
      `auth-schema: ${COMMITTED} is stale — run 'bun scripts/auth-schema.ts --write' and commit it`,
    )
