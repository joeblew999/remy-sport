...
| File | Owner |
|---|---|
| `src/db/auth-schema.ts` | generated by the `auth` CLI |
...
**Postscript (2026-08-23): the 1.4 pin is lifted — we are on 1.7.1.**

This section previously read "upgrading Better Auth is currently blocked at the 1.4 line", because `@better-auth/cli` published no 1.7.x and bundles its own `@better-auth/core`, so installing it beside `better-auth@1.7.1` hoisted core 1.4.22 against a 1.7.1 runtime.

That diagnosis was right and the conclusion was wrong: **the CLI was renamed.** `@better-auth/cli` is frozen at 1.4.22 permanently; the package is now plain `auth`, versioned in lockstep with core. Watching the old name made a rename look like abandonment. `bun pm why @better-auth/core` now shows a single 1.7.1 required by both `better-auth` and `auth`, so the hoisting hazard is structurally gone rather than worked around.

The other half of the block was real and needed [migration 0007](../../../src/db/migrations/0007_account_issuer.sql): 1.7 keys accounts on `(issuer, account_id)` and matches `sign-in/email` against `local:credential`, so every sign-in failed until `account.issuer` existed and was backfilled. SQLite cannot add a NOT NULL column or promote one in place, so 0007 rebuilds the table — converting in place rather than clearing it, which is what 0003 above said to do once the clearing licence expired.

**Generalisable lesson:** `deps:outdated` cannot see a rename. A dependency that looks frozen deserves one check of whether it still lives under the same name before its version ceiling is written into an ADR.

### 9f. Database joins are on, and were measured before being trusted

`createAuth` sets `advanced: { database: { joins: true } }` (stable in 1.7; it was `experimental: { joins: true }` before). Session lookups fetch session and user in one query instead of two.

**The flag is not self-verifying.** The Drizzle adapter implements joins through Drizzle's relational query API — `db.query[model].findFirst({ with })` — which needs the `relations()` exports that `auth-schema.ts` generates and `schema.ts` re-exports. If it cannot find them it logs *"The model X was not found in the query object … Falling back to regular query"* and serves the request correctly anyway. Setting the flag and seeing green tests therefore demonstrates nothing: the fallback is invisible from outside.

Confirmed with a negative control rather than by reading the source. With the schema stripped from the `drizzle()` call, sign-in logs that error for `user` and `session` — proving the join path is reached on the session hot path and that the log line fires when it degrades. With the real schema, zero such lines.

Measured on local D1, `/api/auth/get-session`, 200 requests after a 20-request warm-up, two runs each:

| | run 1 | run 2 |
|---|---|---|
| `joins: true` | 13.34 ms/req | 12.93 ms/req |
| `joins: false` | 14.55 ms/req | 14.56 ms/req |

≈1.4 ms/req, about 10%, reproducible across runs. Two caveats keep this honest: the figure includes curl and HTTP overhead, so the saving as a fraction of *database* time is larger; and local D1 is on-disk SQLite where a "round trip" is a function call, whereas remote D1 is network-attached, so this understates the production benefit rather than overstating it.
