...
| Work | State | Next step / owning plan |
| --- | --- | --- |
| Adopting shadcn-places | Proposed; the service is live, nothing here depends on it | [Adopting shadcn-places](2026-09-10-01-adopting-shadcn-places.md). Places moved out to their own repo and Worker — [shadcn-places](https://github.com/joeblew999/shadcn-places), live at https://shadcn-places.gedw99.workers.dev — where the source measurements and the corrections now live. This plan is only about what *this* app stops owning. Open: whether places stay vocabularies here, the `CITY_CODES` `z.enum` blocker, and two live bugs (`tl` resolving to `en-US`, and 1,199 English-in-non-Latin name cells). |
...
| Browser tier flakiness | Both causes found and fixed 2026-09-10 | [The browser tier fails differently every time](2026-09-09-18-browser-tier-flakiness.md). A different spec nearly every run; at least six deploy attempts lost. **Cause one:** Vite discovered `workbox-window` through a dynamic import mid-run and reloaded every open page, discarding whatever a test had navigated to — named in `optimizeDeps.include`. **Cause two:** the people picker's list arriving moved the submit button between `mousedown` and `mouseup`, so the browser fired `click` on the form and nothing submitted — a product defect a reader meets too. Four ruled-out theories kept with their evidence. |
