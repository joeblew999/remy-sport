...
| Installed-app Back | Proposed; **scope corrected 2026-09-10** | [Back navigation](2026-09-09-10-installed-app-back-navigation.md). Not a phone problem: **every installed surface loses *return*** — Android, iOS, Windows, Mac and the intended Tauri app for Windows and Mac, where a 1280px window has no chrome and no system back. Phones additionally lose *hierarchy*, because crumbs are folded away below `sm`; the registry's `breadcrumb-responsive` collapses rather than hides and answers that half. The `from=` trail already exists in `routeHref` and survives tab and filter changes, so the expensive part is built. `minimal-ui` and `history.back()` are considered and rejected in the plan. |
