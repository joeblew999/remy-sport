...
| Work | State | Next step / owning plan |
| --- | --- | --- |
| Standalone places service | Proposed; five sources measured, decisions taken, arranged to run unattended | [A standalone places service, in every language](2026-09-09-16-pretranslated-reference-data.md). **Independent of this app** — its own public repo, Worker and database, holding countries, subdivisions and cities in every language the sources carry, adopted here later under a separate plan. CLDR for countries, dr5hn plus GeoNames for subdivisions, Wikidata for cities. PO decided: publish separately, dr5hn's ODbL accepted, transliteration deferred but not foreclosed. App-side findings it deliberately does **not** own are listed under "Not this plan". |
