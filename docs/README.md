...
| --- | --- | --- |
| `ops api` — one executor over the router | Planned, not started | [ops api](2026-09-11-02-ops-api.md). One way to call a procedure from outside the app, derived from the router: a terminal executor for a person, and the same executor behind a tag filter for help-tools' assistant surface. Absorbs the F5 follow-up from [unify server routing](done/2026-09-11-01-unify-server-routing-orpc.md) — help-tools' six agent operations stop being a hand-kept list. Precedes the notifications work, which needs it to debug the scheduler. |
| Notifications, end to end | Planned, not started | [Unify notifications](2026-09-11-03-notifications-unification.md). Content model, a single trigger path, and a Durable Object scheduler replacing the cron. Collapses the game, meeting and reminder mail templates into one generic template — the three entries the registry in `src/mail/templates/index.ts` is built to lose. |
| TanStack Router | Planned, not started | [Migrate to TanStack Router](2026-09-11-04-tanstack-router-migration.md). Replaces the 315 hand-written lines of `src/web/lib/router.tsx` and unifies routing with TanStack Query. This is the decision the "No router library" register entry below has been holding open. |
