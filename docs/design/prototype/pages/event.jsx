// claude-design: app/pages/event.jsx
const { useState: uS_evt } = React;
const { Icon: Ico_evt } = window.RemyShell;

function EventPage({ id, goto, lang }) {
  const allEvents = window.RemyData.useEvents();
  const e = window.RemyData.useEvent(id) || allEvents[0];
  const [tab, setTab] = uS_evt('overview');
    ...
    <>
      <div className="event-hero">
        ...
        <div className="event-actions">
          ...
          <button className="btn primary"><Ico_evt name="follow"/>Follow event</button>
          <button className="btn">Add to calendar</button>
          <button className="btn"><Ico_evt name="share"/>Share</button>
        </div>
      </div>
...
function EventOverview({ e, goto }) {
  ...
  const standings = window.RemyData.useStandings();
  return (
...
function StandingsTable() {
  const S = window.RemyData.useStandings();
  return (
    <div className="page-inner">
      <div className="dash-card">
...
window.RemyPages = Object.assign(window.RemyPages || {}, { EventPage, EventOverview, StandingsTable, SchedulePlaceholder });
