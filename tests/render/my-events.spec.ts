...
test.describe("My Events", () => {
  test("splits what you organise from what you follow", async ({ page }) => {
    await seed(page, [
      { id: "evt_002", relation: "OWNER" as const },
      { id: "evt_001", relation: "CO_ORGANIZER" as const },
      { id: "evt_003", relation: "FOLLOWER_EVENT" as const },
    ])
    ...
    await expect(organising.getByTestId("my-event-evt_002")).toBeVisible()
    await expect(organising.getByTestId("my-event-evt_001")).toBeVisible()
    await expect(organising.getByTestId("my-event-evt_003")).toHaveCount(0)

    await expect(following.getByTestId("my-event-evt_003")).toBeVisible()
    await expect(following.getByTestId("my-event-evt_002")).toHaveCount(0)
  })
  ...
  test("says so for each half separately, rather than showing an empty page", async ({ page }) => {
    ...
    await seed(page, [{ id: "evt_002", relation: "OWNER" as const }])
    await visit(page, "myEvents")

    await expect(page.getByTestId("my-event-evt_002")).toBeVisible()
