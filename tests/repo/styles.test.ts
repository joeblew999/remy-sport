...
/**
 * What `styles.css` pulls in, because a token is resolved across the imports and
 * not within one file.
 *
 * `fonts.css` is generated by `bun run ops fonts` and imported at the top of
 * styles.css. It defines `--font-scripts` — every self-hosted script face,
 * derived from the locales the model declares — which styles.css interpolates
 * into `--font-sans`. Reading only styles.css would call that token undefined
 * and be wrong about it.
 */
const FONTS_PATH = "src/web/fonts.css"
const fontsCss = readFileSync(FONTS_PATH, "utf8")
const resolvedCss = `${fontsCss}\n${css}`

rule("shared CSS tokens resolve or provide a fallback", undefinedTokens(resolvedCss),
  `${CSS_PATH} uses tokens that nothing defines: ${undefinedTokens(resolvedCss).join(", ")}`)

/**
 * Every face this repository vendors is one the page can actually ask for.
 *
 ...
 * twenty-seven-locale release taught `ops fonts` to fetch Arabic, Devanagari and
 * Bengali, and `--font-sans` stayed a hand-typed list of Inter and Thai. 510KB
 * sat in `src/web/fonts/` that no element referenced, so no browser ever
 * requested it and those scripts rendered in the system face. Every check was
 * green: the bytes were there, the CSS was valid, nothing was tofu, and the
 * self-hosting was inert.
 *
 * The stack is generated from the same list now, so this cannot drift by
 * forgetting. What it can still do is drift by *editing* — someone dropping
 * `var(--font-scripts)` out of the tail — and that is what this catches.
 *
 * The mono face is exempt for the reason it always is: it is only ever used for
 * code and digits and carries no script beyond Latin.
 */
const vendored = [...new Set([...stripComments(fontsCss).matchAll(/font-family:\s*'([^']+)'/g)].map(m => m[1]!))]
  .filter(family => family !== "IBM Plex Mono")
const resolvedSans = (() => {
  const sans = stripComments(css).match(/--font-sans\s*:\s*([^;]+)/)?.[1] ?? ""
  const scripts = stripComments(fontsCss).match(/--font-scripts\s*:\s*([^;]+)/)?.[1] ?? ""
  return sans.replace(/var\(\s*--font-scripts\s*\)/, scripts)
})()
const unreachable = vendored.filter(family => !resolvedSans.includes(`'${family}'`))

rule("every self-hosted face is reachable from the font stack", unreachable,
  `${FONTS_PATH} vendors face(s) that --font-sans never asks for: ${unreachable.join(", ")}\n\n` +
  `A browser downloads a face when an element requests its family and a character\n` +
  `falls in its unicode-range. A vendored family named in no stack is bytes in the\n` +
  `repository that no reader ever fetches, and a script that silently renders in\n` +
  `whatever the operating system supplies.\n\n` +
  `The stack is generated: run \`bun run ops fonts\` and keep var(--font-scripts)\n` +
  `in the tail of --font-sans in ${CSS_PATH}.`,
  `check-styles: ${vendored.length} self-hosted face(s), each reachable from --font-sans`)
...
/* The tails are load-bearing: the self-hosted scripts are what render Thai,
   Arabic, Devanagari and Bengali, and system-ui is what renders CJK, which is
   deliberately not self-hosted. Drop either and a declared locale becomes tofu
   or falls back to whatever the device happens to have.

   `--font-sans` carries the generated `var(--font-scripts)` — every script face
   the model's locales imply — rather than a list typed here, which is the thing
   that fell behind by three languages. `--font-thai` names Thai directly and on
   purpose: it is the Thai-FIRST face, for a line that is Thai with a Latin word
   or two in it, so its order is the point rather than its membership.

   The mono token is exempt on purpose — only ever code and digits. */
const tailless = [
  ...(/var\(\s*--font-scripts\s*\)/.test(stripComments(css).match(/--font-sans\s*:\s*([^;]+)/)?.[1] ?? "")
    ? [] : ["--font-sans (lost var(--font-scripts))"]),
  ...["--font-sans", "--font-thai"].filter(token => {
    const value = stripComments(css).match(new RegExp(`${token}\\s*:\\s*([^;]+)`))?.[1] ?? ""
    return !value.includes("system-ui")
  }).map(token => `${token} (lost system-ui)`),
  ...(stripComments(css).match(/--font-thai\s*:\s*([^;]+)/)?.[1] ?? "").includes("'Noto Sans Thai'")
    ? [] : ["--font-thai (lost 'Noto Sans Thai')"],
]
rule("the text font tokens keep the script faces and system-ui in their tails", tailless,
  `Font tokens in ${CSS_PATH} lost part of their tail: ${tailless.join(", ")}`)
